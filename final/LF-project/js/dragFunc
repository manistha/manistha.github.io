function DragEvents() {
    ...
    //Start Drag
    this.startDrag = function(e) {  //e => mouseEvent
        var t = e.target;       
    ...
    this.endDrag = function() {
        if ( !that.el.children[0] ) 
            return;
        that.parentNew = that.getDroppable(that.el.children[0], this.parentOld);
        ...
        if (dropout.children.length == 104){ 
            ...
            UpdateScore();
            bgSound = new Audio('bg-music.mp3');
    ...
    //Check whether card is droppable or not
    this.getDroppable = function(target, source) {  //parameters are target and old parent
        if (!target) return;
        ...
        if (!this.container || this.container == source) 
            return;
        if ( !this.container.children[0] ) //If the container is empty
